...
struct ICloudItemView: View {
    ...
    func download() {
        ...
        data.concurrentQueue.async { [self] in
            ...
            do {
                data.notes[index].content = try CoordinatedFile.read(URL(fileURLWithPath: downloadedFilePath))
            }
            catch {
                /* error handling here */
                print("Unexpected error: \(error).")
            ...
            do {
                data.notes[index].date = try (FileManager.default.attributesOfItem(atPath: downloadedFilePath)[.creationDate] as? Date) ?? Date()
            }
            catch {
                /* error handling here */
                print("Unexpected error: \(error).")
